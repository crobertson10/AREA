...
router.get('/auth/slack', async (req, res) => {
    const request = `https://slack.com/oauth/authorize?client_id=${process.env.SLACK_CLIENT_ID}&scope=users.profile:read+groups:write+channels:write+channels:read+char:write:user&redirect_uri=http://localhost:8080/dashboard`;
